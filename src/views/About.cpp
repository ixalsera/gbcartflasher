...
About::About(QWidget* parent) : QDialog(parent)
{
    ...
    name = new QLabel(tr("GB Cart Flasher for Win9x, Win2k, WinXP version ") +
                      VER + " (32-bit)", box);
    labels->addWidget(name);
    copy = new QLabel("Copyright (c) 2005-2007 Kraku & Chroost", box);
    labels->addWidget(copy);
    email = new QLabel(tr("e-mail: <a href=mailto:gbflasher@interia.pl>gbflasher@interia.pl</a>"), box);
    labels->addWidget(email);
    translator = new QLabel(tr("Translation by: Chroost"), box);
    labels->addWidget(translator);
    desc1 = new QLabel(
        tr("This program and device, called set, enables to:\n"
    ...
    desc2 = new QLabel(
        tr("Using this set for purposes other than previously listed,\n"
            "especially for making illegal copies of copyrighted games is strictly prohibited!"),
        box);
