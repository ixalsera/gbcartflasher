...
void Gui::startup_info() const
{
    ...
    if (Settings::commanual == false)
    {
        ...
        {
            if (Logic::read_status(port, settings->getCom(i).toUtf8(), NREAD_ID, 0x00, 0x00, &status) == true)
            {
                which_port = i;
                break;
            }
        ...
        else
        {
            ...
            const QString versionFormatted = QString::asprintf(" %d%d.%d%d", status.ver_11, status.ver_12,
                                                               status.ver_21, status.ver_22);
            console->print(tr("Device firmware version:") + versionFormatted);
        }
    }
    console->line();
}

void Gui::show_info() const
{
    status_t status;
    AbstractPort* port = create_port();
    const int return_code = Logic::read_status(port, settings->getCom().toUtf8(), READ_ID,
                                               settings->getMbc(),
    ...
    if (return_code == true) /* no error */
    {
        ...
        QString tmp = QString::asprintf(" %d%d.%d%d", status.ver_11, status.ver_12,
                                        status.ver_21, status.ver_22);
        console->print(tr("Device firmware version:") + tmp);
        console->print("\n" + tr("--Cartridge information--"));
        tmp = QString::asprintf(" 0x%x", status.manufacturer_id);
        console->print(tr("FLASH memory manufacturer ID:") + tmp);
        tmp = QString::asprintf(" %s", status.manufacturer);
        console->print(tr("FLASH memory manufacturer name:") + tmp);
        tmp = QString::asprintf(" 0x%x", status.chip_id);
        ...
        if (status.logo_correct == 1)
        {
            ...
            tmp = QString::asprintf(" %s", status.game_name);
            ...
            tmp = QString::asprintf(" %s", status.typ);
            console->print(tr("Cartridge type:") + tmp);
            tmp = QString::asprintf(" %s", status.rom_size);
            console->print(tr("ROM size:") + tmp);
            tmp = QString::asprintf(" %s", status.ram_size);
            console->print(tr("RAM size:") + tmp);
            tmp = QString::asprintf(" 0x%x", status.crc16);
...
void Gui::read_flash()
{
    file_name = QFileDialog::getSaveFileName(this, tr("Write FLASH to..."), path,
...
void Gui::write_flash()
{
    file_name = QFileDialog::getOpenFileName(this, tr("Read FLASH from..."), path,
                                             tr("GB Rom Dumps (*.gb *.gbc *.sgb)"));
    path = Logic::get_path(file_name);
    if (file_name != "")
    {
        ...
        {
            const long bytes_count = Logic::file_size(thread_WFLA->file);
            thread_WFLA->page_count =
                static_cast<short>((bytes_count % 16384L)
                                       ? (bytes_count / 16384 +
                                           1)
                                       : (bytes_count / 16384L));
            kilobytes_count =
                static_cast<short>((bytes_count % 1024L)
                                       ? (bytes_count / 1024 +
                                           1)
                                       : (bytes_count / 1024L));
        }
        else if ((kilobytes_count = Logic::flash_file_size(thread_WFLA->file)) != false)
...
void Gui::read_ram()
{
    file_name = QFileDialog::getSaveFileName(this, tr("Write RAM to..."), path,
                                             tr("GB Save (*.sav)"));
    path = Logic::get_path(file_name);
    if (file_name != "")
    {
...
void Gui::write_ram()
{
    file_name = QFileDialog::getOpenFileName(this, tr("Read RAM from..."), path,
                                             tr("GB Save (*.sav)"));
    path = Logic::get_path(file_name);
    if (file_name != "")
    {
        ...
        if (settings->isAuto() == false)
        {
            if (const long bytes_count = Logic::file_size(thread_WRAM->file); bytes_count == 2048)
            {
                thread_WRAM->_2k = 1;
            ...
            else
            {
                ...
                thread_WRAM->page_count =
                    static_cast<short>((bytes_count % 8192L)
                                           ? (bytes_count / 8192L +
                                               1)
                                           : bytes_count / 8192L);
                kilobytes_count =
        ...
        else if ((kilobytes_count = Logic::ram_file_size(thread_WRAM->file)) != false)
...
void Gui::setProgress(const int ile, const int max) const
{
...
void Gui::setEnabledButtons(bool state) const
{
    ...
    //this buttons needs to be disabled if no ram is avilable
...
void Gui::setRamButtons() const
{
    if (status_btn->isEnabled())
        setEnabledButtons(true);
}

void Gui::print_error(const int err) const
{
    switch (err)
    {
    case FILEERROR_O:
    ...
    case true:
        console->print(tr(">Success!")); /* success is not an error code */
        break;
    }
