...
void ReadFlashThread::run()
{
    ...
    int character, data_type = 0x00, received = 0, retries = 0, prv_count = 0;
    config_t cfg;
    cfg.operation = RROM;
    cfg.mbc = mbc;
    ...
    do
    {
        if (received != 0) /* if first packet, don't need to get it */
            character = port->receive_packet(packet);
        ...
        {
            /* valid data packet */
            const int page_number = received / 256; /* 256 packets = page */
            const int packet_number = received % 256;
            retries = 0;
                ...
                && page_number == packet[4] * 256 + packet[5])
            {
                ...
                emit set_progress(received, page_count * 256 - 1);
            }
    ...
    if (received * 64 == page_count * 16384)
    {
        port->close_port();
        fclose(file);
        emit error(true);
