...
SerialPort::SerialPort() : AbstractPort()
{
    descriptor = -1; // Descriptor -1 is invalid - something like NULL
}


bool SerialPort::open_port(const QString port_name)
{
...
int SerialPort::receive_char()
{
    time_t tp = time(nullptr); /* save current time */
    ...
    else /* if packet, read rest of it */
    {
        ...
        do
        {
            ...
            if (read_count > 0)
            {
                ...
                tp = time(nullptr); /* reset wait timer */
            }
        }
        while (data_left != 0 && time(nullptr) - tp < SLEEPTIME);
        /*
        ...
        if (data_left > 0)
            return TIMEOUT;
        return DATA;
        /* DATA signals read of full packet */
    }
}
