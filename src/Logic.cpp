 ...
 *
 * Originally from GB CPU Manual; verified and updated from
...
desc_t carts[] = {
    ...
    {0xfd, "Bandai TAMA5"},
    {0xfe, "Hudson HuC-3"},
    {0xff, "Hudson HuC-1+RAM+BATT"},
};

/**
 * List of ROM sizes.
 *
 * Verified and updated from Pan Docs (https://gbdev.io/pandocs/The_Cartridge_Header.html)
 */
desc_t rom_sizes[] = {
    {0x00, "32KB"},
    {0x01, "64KB"},
    ...
    {0x05, "1MB"},
    {0x06, "2MB"},
    {0x52, "1.1MB"}, // Not that we will ever encounter this it seems but for posterity...
    {0x53, "1.2MB"}, // Not that we will ever encounter this it seems but for posterity...
    {0x54, "1.5MB"} // Not that we will ever encounter this it seems but for posterity...
};
...
desc_t ram_sizes[] = {
    {0x00, "0KB"},
    {0x01, "2KB"}, // Not that we will ever encounter this it seems but for posterity...
    {0x02, "8KB"},
    {0x03, "32KB"},
    {0x04, "128KB"}
};
...
int Logic::read_status(
    AbstractPort* port,
    const char* port_name,
    const char id,
    const char mbc,
    const char alg,
    status_t* flasher_stat)
{
    assert(port != nullptr);
    ...
    packet[3] = mbc;
    packet[4] = alg;
    const unsigned short crc16 = generate_crc16(packet);
    ...
    {
        int i;
        constexpr int producers_count = std::size(producers);
        constexpr int types_count = std::size(carts);
        constexpr int types_count_rom = std::size(rom_sizes);
        constexpr int types_count_ram = std::size(ram_sizes);
        ...
        for (int inc = 0; inc < 16; inc++)
            if (!isalnum(flasher_stat->game_name[inc])
                || flasher_stat->game_name[inc] == '\n')
                flasher_stat->game_name[inc] = ' ';
...
unsigned short Logic::generate_crc16(unsigned char packet[PACKETSIZE])
{
...
void Logic::fill_data_packet(
    unsigned char packet[PACKETSIZE],
    unsigned char data[FRAMESIZE],
    const char packet_type,
    const unsigned char packet_index,
...
int Logic::check_packet(unsigned char packet[PACKETSIZE])
{
    const unsigned short crc16 = generate_crc16(packet);
...
long Logic::file_size(FILE* file)
{
    long size = 0;
...
short Logic::flash_file_size(FILE* file)
{
    /* file_size w kB */
    switch (const long mem_size = file_size(file))
    {
    case 32 * 1024:
    case 64 * 1024:
    case 128 * 1024:
    ...
    case 4096 * 1024:
    case 8192 * 1024:
        return static_cast<short>(mem_size / 1024L);
    default:
        return false;
    }
}

short Logic::ram_file_size(FILE* file)
{
    switch (const long mem_size = file_size(file))
    {
    case 2 * 1024:
...
bool Logic::send_start_packet(AbstractPort* port, config_t cfg)
{
    unsigned char packet[PACKETSIZE] = {};
    packet[0] = DATA;
    packet[1] = CONFIG;
...
QString Logic::get_path(const QString& name)
{
    if (int end; (end = name.lastIndexOf("/")) != -1)
    {
