...
** Author: Kraku
*****************************************************************************/
#include "headers/ReadRamThread.h"
...
void ReadRamThread::run()
{
    ...
    int n, data_type = 0x00, received = 0, retries = 0, prv_count = 0;
    config_t cfg;
    cfg.operation = RRAM;
    cfg.mbc = mbc;
    ...
    do
    {
        ...
        {
            /* good packet */
            const int page_number = received / 128; /* each ram page consist of 128 packets */
            const int packet_number = received % 128;
            retries = 0;
                ...
                && page_number == packet[4] * 256 + packet[5])
            {
                ...
                emit set_progress(received + 1, _2k ? 32 : page_count * 128);
            }
    ...
    if (received * 64 == page_count * 8192)
    {
        port->close_port();
        fclose(file);
        emit error(true);
